...
router.post('/remove', async (req, res) => {
  ...
  const updatedUser = new User({
    ...user,
    favoriteMemes: {
      items: user.favoriteMemes.items.filter(m => m.memeId.toString() !== req.body.id.toString()),
    },
  })
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const user = await req.user.populate('favoriteMemes.items.memeId').execPopulate()
